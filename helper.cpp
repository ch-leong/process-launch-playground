...
bool EnableDebugPrivileges(std::string& errorMsg)
{
    HANDLE token;
    LUID SeDebugNameValue;
    TOKEN_PRIVILEGES TokenPrivileges;

    BOOL ret = OpenProcessToken(GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, &token);
    if (!ret)
    {
        errorMsg = "OpenProcessToken failed with " + std::to_string(GetLastError());
        return false;
    }

    ret = LookupPrivilegeValue(NULL, SE_DEBUG_NAME, &SeDebugNameValue);
    if (!ret)
    {
        errorMsg = "LookupPrivilegeValue failed with " + std::to_string(GetLastError());
        CloseHandle(token);
        return false;
    }
    ...
    ret = AdjustTokenPrivileges(token, FALSE, &TokenPrivileges, sizeof(TOKEN_PRIVILEGES), NULL, NULL);
    if (!ret)
    {
        errorMsg = "AdjustTokenPrivileges failed with " + std::to_string(GetLastError());
    }
    CloseHandle(token);
    return ret;
}
...
bool IsRunningElevated(bool& isElevated, HANDLE token)
{
    ...
    BOOL ret = GetTokenInformation(token, TokenElevation, &elevation, sizeof(elevation), &cbSize);
    if (!ret)
    {
        std::cout << "GetTokenInformation error with: " + std::to_string(GetLastError()) << "\n";
        return false;
    }
...
bool IsRunningElevated(bool& isElevated, DWORD processId)
{
    std::string errorMsg;
    bool ret = true;
    ret = EnableDebugPrivileges(errorMsg);
    if (!ret)
    {
        std::cout << errorMsg << "\n";
        return false;
    ...
    if (!processHandle)
    {
        std::cout << "OpenProcess error with: " + std::to_string(GetLastError()) << "\n";
        return false;
    }

    HANDLE token;
    DWORD desiredAccess = TOKEN_QUERY;
    ret = OpenProcessToken(processHandle, desiredAccess, &token) == TRUE;
    if (!ret)
    {
        std::cout << "OpenProcessToken error with: " + std::to_string(GetLastError()) << "\n";
    }
    ret = IsRunningElevated(isElevated, token);
    CloseHandle(token);
    return ret;
}
...
void PrintTokenPrivilege(HANDLE token)
{
    ...
    DWORD dwTokLen = 0x0;
    GetTokenInformation(token, TokenPrivileges, NULL, 0x0, &dwTokLen);
    DWORD dwRetLen = 0x0;
    PTOKEN_PRIVILEGES tp = (PTOKEN_PRIVILEGES)malloc(dwTokLen);

    BOOL ret = GetTokenInformation(token, TokenPrivileges, (LPVOID)tp, dwTokLen, &dwRetLen);
    if (!ret)
    {
        std::cout << "GetTokenInformation error with: " + std::to_string(GetLastError()) << "\n";
        return;
    ...
    for (DWORD c = 0; c < tp->PrivilegeCount; c++)
    {
        ...
        std::wstring lpName(1000, L' ');
        std::wstring lpDisplayName(1000, L' ');
        ...
        ret = LookupPrivilegeNameW(NULL, &tp->Privileges[c].Luid, &lpName[0], &dwName);
        if (!ret)
        {
            std::cout << "LookupPrivilegeNameW error with: " + std::to_string(GetLastError()) << "\n";
            continue;
        }
        std::wcout << L"Name: " << lpName << std::endl;
        if (lpName[0] != '0')
        {
            privileges.push_back(lpName);
        }

        // Get the description / display for the privilege by its name.
        ret = LookupPrivilegeDisplayNameW(NULL, &lpName[0], &lpDisplayName[0], &dwDisplayName, &dwLangId);
        if (!ret)
        {
            std::cout << "LookupPrivilegeNameW error with: " + std::to_string(GetLastError()) << "\n";
            continue;
